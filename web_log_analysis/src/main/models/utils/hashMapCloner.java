...
public class hashMapCloner {
    ...
    public static final <K, V> HashMap<K, V> deepCopy(HashMap<K, V> original) {
        if(original == null) return null;
        HashMap<K, V> copy = new HashMap<>();

        for (Map.Entry<K, V> entry : original.entrySet()) {
            copy.put(entry.getKey(), hashMapCloner.deepCopyValue(entry.getValue()));
        }
    ...
    private static final <V> V deepCopyValue(V value) {
        if (value == null) {
            ...
            return (V) hashMapCloner.deepCopy((HashMap<?, ?>) value); // Recursively copy HashMap
        } else if (value instanceof ArrayList) {
            return (V) hashMapCloner.deepCopyList((ArrayList<?>) value); // Recursively copy ArrayList
    ...
    private static final <V> ArrayList<V> deepCopyList(ArrayList<V> original) {
        if(original == null) return null;
        ArrayList<V> copy = new ArrayList<>();
        
        for (V item : original) {
            copy.add(hashMapCloner.deepCopyValue(item)); // Recursively deep copy elements
        }
    ...
    private static final <K, V> AbstractMap.SimpleEntry<K, V> deepCopySimpleEntry(AbstractMap.SimpleEntry<K, V> entry) {
        ...
        return new AbstractMap.SimpleEntry<>(entry.getKey(), hashMapCloner.deepCopyValue(entry.getValue()));
    }
    ...
    private static final <T> ObservableList<T> deepCopyObservableList(ObservableList<T> original) {
        ...
        for (T item : original) {
            copy.add(hashMapCloner.deepCopyValue(item));
        }
